...
import { ArrowLeft, Clock, Zap, TrendingDown, Calendar, Check, Loader2 } from 'lucide-react';
...
const TariffOptimizer: React.FC<Props> = ({ onBack }) => {
    ...
    const handleScheduleWashingMachine = async () => {
        if (!home?.id || scheduling || scheduled) return;
        setScheduling(true);
        setScheduleError('');
        try {
            // Find the washing machine appliance
            ...
            const washingMachine = appliances?.[0];
            if (!washingMachine) {
                // Try by category
                const { data: byCat } = await supabase
                    ...
                    .select('id, name')
                    .eq('home_id', home.id)
                    .eq('category', 'washing_machine')
                    .eq('is_active', true);
                if (!byCat?.[0]) throw new Error('No washing machine found. Add one in Control first.');
            }

            const applianceId = washingMachine?.id || (await supabase
                .from('appliances')
                .select('id')
                .eq('home_id', home.id)
                .eq('category', 'washing_machine')
            ...
            if (!applianceId) throw new Error('No washing machine found.');
            ...
            await supabase.from('appliances')
                .update({ status: 'SCHEDULED', schedule_time: '22:00' })
                .eq('id', applianceId);
        ...
        <div className="pt-8 pb-32 px-5 overflow-y-auto h-full no-scrollbar bg-slate-50">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white rounded-[2rem] shadow-soft border border-slate-100 p-5 relative overflow-hidden">
                    ...
                    <button
                        onClick={handleScheduleWashingMachine}
                        disabled={scheduling || scheduled}
                        className={`w-full mt-4 py-3 rounded-xl text-sm font-bold shadow-lg flex items-center justify-center gap-2 transition-colors ${scheduled
                    ...
                    {scheduleError && <p className="text-xs text-rose-500 mt-2 text-center">{scheduleError}</p>}
                </div>
