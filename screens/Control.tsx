...
import { DBAppliance, ApplianceCategory } from '../types/database';
...
// ── Appliance Tile ─────────────────────────────────────────────────
interface ApplianceTileProps { appliance: DBAppliance; compact: boolean; costPerHour: number; isLoading: boolean; onToggle: () => void; onSchedule: () => void; onEdit: () => void; onDelete: () => void; delay: number; }

const ApplianceTile: React.FC<ApplianceTileProps> = ({ appliance, compact, costPerHour, isLoading, onToggle, onSchedule, onEdit, onDelete, delay }) => {
      ...
      <div className={`z-10 ${compact ? 'mt-auto' : 'mt-2'}`}>
        ...
        {isOn ? (
          ...
          <div className={`flex items-center gap-1 font-medium text-indigo-500 bg-indigo-50 rounded-lg self-start inline-flex ${compact ? 'text-[9px] px-1.5 py-0.5' : 'text-xs px-2 py-1'}`}>
            <Clock className={compact ? 'w-2 h-2' : 'w-3 h-3'} /> Scheduled
          </div>
        ) : (
...
// ── Schedule Modal ─────────────────────────────────────────────────
interface ScheduleModalProps { homeId: string; appliance: DBAppliance; onClose: () => void; onSaved: () => void; }

const ScheduleModal: React.FC<ScheduleModalProps> = ({ homeId, appliance, onClose, onSaved }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} onClick={e => e.stopPropagation()} className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-md">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div><h2 className="text-xl font-bold text-slate-800">Schedule</h2><p className="text-sm text-slate-400">{appliance.name}</p></div>
            <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100"><X className="w-5 h-5 text-slate-400" /></button>
          </div>
          <div className="space-y-5">
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setAction('on')} className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${action === 'on' ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-500'}`}><Power className="w-4 h-4" /> Turn ON</button>
                <button onClick={() => setAction('off')} className={`flex-1 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${action === 'off' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`}><Power className="w-4 h-4" /> Turn OFF</button>
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                ...
                <input type="time" value={time} onChange={e => setTime(e.target.value)} className="flex-1 bg-transparent text-2xl font-bold text-slate-800 outline-none" />
              </div>
              <p className="text-xs text-emerald-600 mt-2 flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-emerald-400" />Off-peak window (₹6.31/kWh)</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Repeat On</label>
              <div className="flex gap-2">
                {DAYS.map(day => (
                  <button key={day} onClick={() => toggleDay(day)} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${selectedDays.includes(day) ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-400'}`}>{day[0]}</button>
                ))}
              </div>
            </div>
            {error && <div className="flex items-center gap-2 text-rose-500 text-sm bg-rose-50 p-3 rounded-xl"><AlertCircle className="w-4 h-4" /> {error}</div>}
